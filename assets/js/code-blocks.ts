...
(function initializeCodeBlocks(): void {
  ...
  // Comment token classes used by Chroma syntax highlighter
  const COMMENT_CLASSES = new Set(['c', 'c1', 'cm', 'cp', 'cs']);

  // Shell-like languages where leading $ prompts should be stripped
  const SHELL_LANGUAGES = new Set(['bash', 'sh', 'shell', 'zsh', 'console', 'terminal']);

  /**
   * Cleans code text for clipboard: strips comment-only lines,
   * removes leading $ prompts from shell blocks, trims trailing whitespace.
   */
  function cleanCodeForCopy(codeElement: HTMLElement, language: string): string {
    const isShell = SHELL_LANGUAGES.has(language);
    const lines = codeElement.querySelectorAll<HTMLElement>('.line');

    // If Chroma line spans exist, process per-line for accurate comment detection
    if (lines.length > 0) {
      const cleaned: string[] = [];
      for (const line of lines) {
        // Check if the entire line is only comment tokens + whitespace
        const children = line.querySelectorAll<HTMLElement>('.cl > span');
        const isCommentOnly = children.length > 0 && Array.from(children).every(
          (span) => COMMENT_CLASSES.has(span.className) || span.textContent?.trim() === ''
        );
        if (isCommentOnly) continue;

        let text = line.textContent ?? '';
        // Strip leading $ prompt (with optional whitespace) for shell languages
        if (isShell) {
          text = text.replace(/^\s*\$\s?/, '');
        }
        cleaned.push(text);
      }
      // Remove trailing empty lines, preserve internal ones
    ...
    // Fallback: plain textContent processing (no Chroma spans)
  ...
  function enhanceCodeBlock(codeElement: HTMLElement): void {
    ...
    copyButton.addEventListener(
      ...
      async (event: MouseEvent) => {
        ...
        const code = cleanCodeForCopy(codeElement, language);
  ...
  /**
   * Wraps bare tables in a responsive container for proper border-radius + scroll
   */
  function wrapTables(): void {
    const tables = document.querySelectorAll<HTMLTableElement>(
      '.blog-post table, .podcast-page table'
    );
